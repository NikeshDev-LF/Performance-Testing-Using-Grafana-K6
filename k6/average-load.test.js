...
export default function () {
    ...
    group('add products to cart', function () {
        ...
        check(response, {
            ...
            // JSON Validation
            'add to cart - valid JSON response': (r) => {
                try {
                    JSON.parse(r.body);
                    return true;
                } catch {
            ...
            'add to cart - content-type is JSON': (r) => r.headers['Content-Type']?.includes('application/json'),
    ...
    group('view cart', function () {
        ...
        check(response, {
            ...
            // JSON Validation
            'view cart - valid JSON response': (r) => {
                try {
                    JSON.parse(r.body);
                    return true;
                } catch {
            ...
            'view cart - content-type is JSON': (r) => r.headers['Content-Type']?.includes('application/json'),
    ...
    group('checkout process', function () {
        ...
        check(response, {
            ...
            // JSON Validation
            'checkout - valid JSON response': (r) => {
                try {
                    JSON.parse(r.body);
                    return true;
                } catch {
            ...
            'checkout - content-type is JSON': (r) => r.headers['Content-Type']?.includes('application/json'),
            ...
            // Error Handling
            'checkout - proper error format on failure': (r) => {
                if (r.status >= 400) {
                    const body = JSON.parse(r.body);
                    return body.error !== undefined || body.message !== undefined;
                }
